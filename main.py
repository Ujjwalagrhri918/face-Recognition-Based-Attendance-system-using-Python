...
firebase_admin.initialize_app(cred,{
    ...
    'storageBucket' : "faceattendancesystem-ccf9b.appspot.com"
})
...
while True:
    ...
    # Resize the captured image to match the dimensions of the background portion
    ...
    if faceCurFrame:
        ...
        if counter != 0:
            if counter == 1:
                ...
                blob = bucket.get_blob(f'Images/{id}.jpg')   # jpg png wala issue hogaya hai

                if blob is not None:
                    array = np.frombuffer(blob.download_as_string(), dtype=np.uint8)
                    imgStudent = cv2.imdecode(array, cv2.COLOR_BGRA2BGR)
                else:
                    print(f'Blob for ID {id} not found.')
